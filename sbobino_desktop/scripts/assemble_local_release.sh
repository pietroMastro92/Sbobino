...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 <version> <arm64-staging-dir> <intel-staging-dir> <output-dir>" >&2
  exit 1
fi
...
python3 - "$OUTPUT_DIR/latest.json" "$VERSION" "$RELEASE_URL" "$ARM_SIGNATURE" "$INTEL_SIGNATURE" <<'PY'
import json
import sys
from datetime import datetime, timezone

path, version, base_url, arm_sig, intel_sig = sys.argv[1:]
document = {
    "version": version,
...
This prerelease adds native macOS Intel distribution alongside Apple Silicon,
with architecture-matched updater and first-launch runtime assets.
EOF

cat <<EOF
Dual-architecture local candidate prepared in:
  $OUTPUT_DIR
